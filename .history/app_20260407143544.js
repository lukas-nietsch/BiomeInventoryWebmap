...
async function init() {
  ...
  let currentCogUrl = "";
  let currentLegendByValue = new Map();
  ...
  async function renderLayer(layerId) {
    ...
    currentLegendByValue = new Map(
      legendItems.map((item) => [Number(item.value), item.legend])
    );

    const metadataRow = metadataRows[layer.index - 1];
    buildMetadataTable(metadataContainer, metadataRow);
    currentPublication = metadataRow ? metadataRow.Publication || "Unknown publication" : "Unknown publication";

    const absoluteCogUrl = new URL(layer.cogPath, window.location.href).href;
    currentCogUrl = absoluteCogUrl;
  ...
  map.on("click", async (event) => {
    ...
    try {
      const values = await MaplibreCOGProtocol.locationValues(
        currentCogUrl,
        { latitude: event.lngLat.lat, longitude: event.lngLat.lng },
        map.getZoom()
      );

      const rawValue = values && values.length > 0 ? values[0] : Number.NaN;
      const classValue = Number.isFinite(rawValue) ? Math.round(rawValue) : Number.NaN;
      const legendEntry = currentLegendByValue.get(classValue);
      ...
      console.error("Could not query COG value on click", error);
    }
  });
